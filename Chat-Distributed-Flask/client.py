...
SERVER_URL = input("Enter server url (default is http://host.docker.internal:5001) :")
if SERVER_URL == '':
...
def get(route):
    return requests.get(f"{SERVER_URL}/{route}")
...
def get_command():
    ...
    if args != None and len(args) > 0:
        ...
        if args[0] == "create":
            ...
            result = get("get-rooms")
            rooms = result.json().get('rooms')
            if rooms == []:
